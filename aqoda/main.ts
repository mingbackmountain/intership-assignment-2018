  ...
  commands.forEach(command => {
    switch (command.name) {
      ...
      case "checkout": {
        ...
        try {
          const checkoutBookingInfo = hotelApplication.checkout(
            keycardId,
            name
          );
      ...
      case "list_guest_by_age": {
        ...
        const guestNames = hotelApplication.getGuestNamesByAge(
          sign,
          parseInt(age)
        );
      ...
      case "book_by_floor": {
        ...
        try {
          ...
          const roomsAndKeycards = hotelApplication.bookByFloorNumber(
            floorNumber,
            customerInfo
          );
          const bookedRoomIds = roomsAndKeycards.map(
            roomAndKeycard => roomAndKeycard.bookedRoomIds
          );
          const keycardIds = roomsAndKeycards.map(
